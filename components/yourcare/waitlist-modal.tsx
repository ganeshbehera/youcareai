...
export function WaitlistModal({ isOpen, onClose, title, description }: WaitlistModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95vw] max-w-lg bg-white border-0 shadow-2xl rounded-2xl overflow-hidden mx-4">
        {/* Premium Header with Gradient */}
        <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 px-4 sm:px-8 py-4 sm:py-6 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-blue-100 text-xs sm:text-sm">Limited to first 10,000 users</p>
              </div>
            </div>
            <DialogDescription className="text-blue-50 leading-relaxed text-sm sm:text-base">
              {description || "Get 6 months free when we launch. Be among the first to prevent disease with AI."}
            </DialogDescription>
          </div>
          
          {/* Floating elements - hidden on small screens */}
          <div className="absolute top-4 right-4 w-20 h-20 bg-white/10 rounded-full blur-xl hidden sm:block"></div>
          <div className="absolute -bottom-4 -right-4 w-16 h-16 bg-white/5 rounded-full blur-lg hidden sm:block"></div>
        </div>
        ...
        {isSubmitted ? (
          <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center">
            ...
            <h3 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3">Welcome to YourCareAI!</h3>
            <p className="text-gray-600 mb-6 leading-relaxed text-sm sm:text-base">
              You're now on our exclusive early access list.<br />
              Check your email for confirmation and next steps.
            </p>
            <div className="flex items-center gap-2 text-xs sm:text-sm text-green-700 bg-green-50 px-3 sm:px-4 py-2 rounded-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-6">
              <Button
                ...
                variant="outline"
                onClick={onClose}
                className="flex-1 h-11 sm:h-12 border-2 border-gray-200 hover:border-gray-300 text-gray-700 font-semibold rounded-xl transition-colors text-sm sm:text-base"
              ...
              <Button
                type="submit"
                className="flex-1 h-11 sm:h-12 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] text-sm sm:text-base"
                disabled={isSubmitting}
              >
                {isSubmitting ? (
                  <>
                    <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 mr-2 animate-spin" />
                    <span className="hidden sm:inline">Securing Your Spot...</span>
        ...
        <div className="bg-gray-50 px-4 sm:px-8 py-3 sm:py-4 border-t border-gray-100">
          <div className="flex items-center justify-center gap-2 text-xs text-gray-600 text-center">
